...
def main(
    ...
    #learner = torch.load( model_name, map_location=device, weights_only=False) work on cpu
    learner.eval()
    learner.to(device)
    ...
    with torch.no_grad():
        ...
        #     # batch shape is [B, 2, C, H, W]
        #     batch = batch[:, 0]   # take only one image
    ...
    print(f"Embedding time: {(t_embed_end - t_embed_start)/60:.2f} minutes")
    ...
    print(f"Clustering time: {(t_cluster_end - t_cluster_start)/60:.2f} minutes")
